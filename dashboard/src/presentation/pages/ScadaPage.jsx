    ...
    <div className="app-container">
      <div className="scada-grid">
        ...
        <Island style={{ borderTop: '3px solid var(--secondary)' }}>
          <IslandHeader icon={User} color="var(--secondary)" title="Paciente" />
          {hasData ? (
            <>
              <StatCard title="ID Paciente" value={data.info.g_patient_id_str?.value || 'N/A'} iconName="Contact" color="var(--secondary)" />
        ...
        <Island style={{ borderTop: '3px solid var(--art-color)' }}>
          <IslandHeader icon={Thermometer} color="var(--art-color)" title="Presiones" />
        ...
        {/* ───── Flows Island ───── */}
        <Island style={{ borderTop: '3px solid var(--tmp-color)' }}>
          <IslandHeader icon={Waves} color="var(--tmp-color)" title="Flujos" />
          ...
          <Island style={{ borderTop: '3px solid var(--primary)' }}>
            ...
            {hasData ? (
              <div className="scada-stats-row">
                ...
                <StatCard title="Remoción neta" value={data.info.c_acc_net_rem_vol_act?.value || 'N/A'} unit={data.info.c_acc_net_rem_vol_act?.unit} iconName="Droplets" color="#22d3ee" />
                <StatCard title="Sub-estado" value={data.info.g_trmt_sub_state_set?.value || 'N/A'} iconName="Activity" color="var(--accent)" />
              </div>
            ) : (
              <div className="empty-state" style={{ padding: '12px' }}><span>Esperando datos...</span></div>
            )}
        ...
        {/* ───── Trends Island (full width) ───── */}
        <div className="scada-full">
          <Island style={{ borderTop: '3px solid var(--accent)' }}>
            <IslandHeader icon={Activity} color="var(--accent)" title="Tendencias" />
              ...
              <div style={{ width: '100%', height: '300px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.history}>
                    ...
                    <Line type="monotone" dataKey="c_pump_bs_bl_flow_act" stroke="var(--art-color)" name="Flujo Sanguíneo" dot={false} strokeWidth={2} strokeDasharray="4 2" isAnimationActive={false} opacity={0.7} />
                    <Line type="monotone" dataKey="c_pump_fs_mid_flow_act" stroke="var(--tmp-color)" name="Flujo Diálisis" dot={false} strokeWidth={2} strokeDasharray="4 2" isAnimationActive={false} opacity={0.7} />
                    <Line type="monotone" dataKey="c_net_rem_flow_act" stroke="var(--fil-color)" name="Rem. Neta" dot={false} strokeWidth={2} strokeDasharray="4 2" isAnimationActive={false} opacity={0.7} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            ) : (
              <div className="empty-state" style={{ padding: '24px' }}>
                <span>{data.history && data.history.length === 1 ? 'Acumulando más datos para la tendencia...' : 'Esperando datos para generar tendencia...'}</span>
              </div>
            )}
          </Island>
        </div>
